...
{
    public class RejectAnticipationRequestUseCaseTests
    {
        [Fact]
        public async Task ExecuteAsync_ShouldRejectAndPersist()
        {
            var id = Guid.NewGuid();
            ...
            var sut = new RejectAnticipationRequestUseCase(repo.Object);

            await sut.ExecuteAsync(id);

            Assert.Equal(RequestStatus.Rejected, entity.Status);
